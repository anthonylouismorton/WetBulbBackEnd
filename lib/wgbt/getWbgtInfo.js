...
const getWbgtInfo = async (lat, lon) => {
  ...
  //Convert time to Arizona time zone. OSHA tool does not allow you to use time zones from around the world
  var date = `${newDate.getMonth()+1}/${newDate.getDate()}/${newDate.getFullYear()}`
  var time = newDate.toLocaleTimeString("en-US", { timeZone: "America/Phoenix", hour12: false}).slice(0,5);
  console.log({
    localDay: localDay,
    localTime: localTime,
    oshaDate: date,
  ...
  let information = {
    ...
    //Converting to Arizona time allows us to always use standard time as Arizona does not use DST
    await page.click('input[id="StandardTime"]');

    // Dates were converted to Arizona time which is in the mountain time zone. -7 is the mountain time zone on the osha page
    const timeZone = await page.$('[name="tz"]');
    await timeZone.select('-7');
    if(lat[0] === '-'){
