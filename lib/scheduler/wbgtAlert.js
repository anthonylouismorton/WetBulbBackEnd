...
const { Alert, Wbgt, User } = require('../models')
const getWbgtInfo = require('../scraper/scraper');

module.exports = () => {  
  cron.schedule('0 */5 * * * *', async () => {
    console.log('getting information every 5 minute minutes')
    var today = new Date();
    ...
    if(hour % 4 === 0){
      ...
      alerts = await Alert.findAll({ 
        where: {frequency: ['4hours', '2hours', 'hourly']},
        include: [{
          model: User, as: 'user',
        }]
      });
